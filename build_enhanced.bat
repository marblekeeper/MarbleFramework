...
:USAGE
echo Usage: build.bat [msvc^|gcc^|test^|ui_test^|sprite_font_editor^|mindmarr^|web^|web_serve^|clean_dlls^|net_*]
echo.
echo Game Builds:
...
REM === Build WASM network client ===
echo [4/4] Building WASM network client with JavaScript WebSocket bridge...

REM Check if network_client.c exists
if not exist "network_client.c" (
    echo [ERROR] network_client.c not found!
    exit /b 1
)

echo [OK] Building with EM_JS WebSocket bindings
echo.

REM Build final WASM with JavaScript WebSocket bridge (no LuaSocket needed)
...
echo.
echo To test:
echo   1. Terminal 1: Run websockify:
echo      pip install websockify
echo      lua server_tcp.lua
echo      websockify 8080 localhost:12345
echo.
echo   2. Terminal 2: Serve WASM:
echo      python -m http.server 8000
echo.
echo   3. Browser:
echo      http://localhost:8000/web_network/
echo.
echo ================================================
exit /b 0
